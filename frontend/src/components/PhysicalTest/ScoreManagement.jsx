...
import { updateTestRecord, deleteTestRecord, addTestRecord, setTestRecords } from '../../store/physicalTestSlice'
...
const ScoreManagement = () => {
  ...
  const synchronizedData = useMemo(() => {
    ...
    testRecords.forEach(record => {
      testRecordsMap.set(record.educationId, record)
    })
    
    return students.map(student => {
      const existingRecord = testRecordsMap.get(student.educationId)
  ...
  const getVisibleTestItems = (grade, gender) => {
    ...
    const stage = primaryGrades.includes(grade) ? 'primary' : 'secondary'
  ...
  const getColumnsWithTestItems = () => {
    ...
    return {
      columns: [...baseColumns, ...testItemColumns],
